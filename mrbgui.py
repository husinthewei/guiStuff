...
class Model:
    def __init__(self):
        ...
        self.mcu = busio.ProtoMcu(boardid=1)
...
#rwh315
    def OnBtnSlow(self,e):
        #boardid= "192.168.1.10"
	#b = busio.UdpBoard(boardid)
	addrS = 0x003
	dataS = 0
	#b.wr(addrS,dataS)
...
class MrbStatsPanel(StatsPanel):
    def __init__(self, parent, model, tabnum, whichmrb):
        ...
        self.AST("nfail");        self.AST(func=m.GetNFail2)
        self.AST("0000");         self.AST(func=mrb.r0000.rfmt)
        self.AST("beef");         self.AST(func=mrb.rbeef.rfmt)
        self.AST("uptime");       self.AST(func=mrb.s3uptime.rfmt)
        self.AST("q0003");        self.AST(func=mrb.q0003.rfmt)
        self.AST("LED pattern");  self.AST(func=mrb.ledpatt.rfmt)
        self.AST("q0005");        self.AST(func=mrb.q0005.rfmt)
        self.AST("DIP sw");       self.AST(func=mrb.dipsw.rfmt)
        self.AST("AD9222 up");    self.AST(func=mrb.ad9222up.rfmt)
        self.AST("AD9287 up");    self.AST(func=mrb.ad9287up.rfmt)
        self.AST("DRS4 PLL Lck"); self.AST(func=mrb.drs4plllck.rfmt)
        self.AST("DRS4 DTAP");    self.AST(func=mrb.drs4dtap.rfmt)
        self.AST("Ro Fifo Flgs"); self.AST(func=mrb.rofifoflags.rfmt)
        self.AST("Ro Frame Cnt"); self.AST(func=mrb.roframecount.rfmt)
        self.AST("temp");         self.AST(func=mrb.v5temp.rfmt)
        self.AST("vccint");       self.AST(func=mrb.v5vccint.rfmt)
        self.AST("srclkflip");    self.AST(func=mrb.srclkflip.rfmt)
        self.AST("misaligncnt 0..9"); szr = wx.BoxSizer()
        self.AST(szr=szr, func=self.mrb.misaligncnt0.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt1.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt2.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt3.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt4.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt5.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt6.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt7.rfmt); szr.Add((10,1))
        self.AST(szr=szr, func=self.mrb.misaligncnt8.rfmt); szr.Add((10,1))
...
class DrsStatsPanel(StatsPanel):
    def __init__(self, parent, model, tabnum, whichmrb):
        ...
        self.sizer.Add(szr)
        #
        btn = wx.Button(self, label="DRS0 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo0)
        btn = wx.Button(self, label="DRS0 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge0)
        #
        btn = wx.Button(self, label="DRS1 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo1)
        btn = wx.Button(self, label="DRS1 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge1)
        #
        btn = wx.Button(self, label="DRS2 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo2)
        btn = wx.Button(self, label="DRS2 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge2)
        #
        btn = wx.Button(self, label="DRS3 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo3)
        btn = wx.Button(self, label="DRS3 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge3)
        #
        btn = wx.Button(self, label="DRS4 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo4)
        btn = wx.Button(self, label="DRS4 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge4)
        #
        btn = wx.Button(self, label="DRS5 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo5)
        btn = wx.Button(self, label="DRS5 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge5)
        #
        btn = wx.Button(self, label="DRS6 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo6)
        btn = wx.Button(self, label="DRS6 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge6)
        #
        btn = wx.Button(self, label="DRS7 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo7)
        btn = wx.Button(self, label="DRS7 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge7)
        #
        btn = wx.Button(self, label="DRS8 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo8)
        btn = wx.Button(self, label="DRS8 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge8)
        #
        btn = wx.Button(self, label="DRS9 GO")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnGo9)
        btn = wx.Button(self, label="DRS9 Merge")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMerge9)
        #
        btn = wx.Button(self, label="MrbFifoReset")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnMrbFifoReset)
        btn = wx.Button(self, label="DrainFifo")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnDrainFifo)
        #
        btn = wx.Button(self, label="Draw")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnDraw)
        #
        self.SetSizer(self.sizer)
        self.timer = wx.Timer(self)
        self.Bind(wx.EVT_TIMER, self.OnTimer, self.timer)
...
class MrbTrigStatsPanel(StatsPanel):
    def __init__(self, parent, model, tabnum, whichmrb):
        ...
        self.sizer.Add(szr)
        #
        btn = wx.Button(self, label="Freeze")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnFreeze)
        #
        btn = wx.Button(self, label="UnFreeze")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnUnFreeze)
        #
        btn = wx.Button(self, label="FreezeOnTrigger")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnFreezeOnTrigger)
        #
        self.sizer.Add((1,1))
        #
        btn = wx.Button(self, label="ReadPeds")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnReadPeds)
        #
        btn = wx.Button(self, label="CalcPeds")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnCalcPeds)
        #
        btn = wx.Button(self, label="ZeroPeds")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnZeroPeds)
        #
        btn = wx.Button(self, label="WritePeds")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnWritePeds)
        #
        btn = wx.Button(self, label="SingleAcq")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnSingleAcq)
        #
        btn = wx.Button(self, label="SingleTrig")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnSingleTrig)
        #
        btn = wx.Button(self, label="Draw")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnDraw)
        #
        btn = wx.Button(self, label="Dump")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnDump)
        #
        btn = wx.Button(self, label="TrigAdcTestOn")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnTrigAdcTestOn)
        #
        btn = wx.Button(self, label="TrigAdcTestOff")
        self.sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.OnBtnTrigAdcTestOff)
        #
        self.SetSizer(self.sizer)
        self.timer = wx.Timer(self)
        self.Bind(wx.EVT_TIMER, self.OnTimer, self.timer)
...
class MainWindow(wx.Frame):
    def __init__(self, model):
        self.m = model
        wx.Frame.__init__(self, None, wx.ID_ANY, "MRB GUI", size=(700,700))
        panel = wx.Panel(self)
...
class Model:
    def __init__(self):
        ...
        #self.GetNFail1 = self.mcu.b.nfail    #rwh
        self.mrb2 = busio.Mrb(boardid=5+2, imrb=2)
        #self.GetNFail2 = self.mrb2.b.nfail    #rwh
        self.mrb3 = busio.Mrb(boardid=5+3, imrb=3)
        #self.GetNFail3 = self.mrb3.b.nfail    #rwh
        self.mrb = [None, None, self.mrb2, self.mrb3]
        self._drspedestals = {}  # use [(2,0)] for MRB2 DRS0
        self._drspedestals[(2,0)] = np.zeros((8,1024),dtype=np.int32)
        self._trigspy = [[] for i in range(24)]
